...
def test_generate_symbols_onto_relevant_symbols():
    ...
    actual_symbols = sorted(alphabet.gen_symbols_for_relevant_variables([Var(1), Var(2), Var(3)]))
    ...
    actual_symbols = sorted(alphabet.gen_symbols_for_relevant_variables([Var(2), Var(3)]))
    assert expected_symbols == actual_symbols

    # The last variable is missing
    expected_symbols = sorted((number_to_bit_tuple(i, tuple_size=2) + ('*',)) for i in range(2**2))
    actual_symbols = sorted(alphabet.gen_symbols_for_relevant_variables([Var(1), Var(2)]))
    assert expected_symbols == actual_symbols
    ...
    actual_symbols = sorted(alphabet.gen_symbols_for_relevant_variables([Var(1), Var(3)]))
    assert expected_symbols == actual_symbols

    # Two are missing
    expected_symbols = sorted(('*', '*', i) for i in range(2))
    actual_symbols = sorted(alphabet.gen_symbols_for_relevant_variables([Var(3)]))
    assert expected_symbols == actual_symbols
