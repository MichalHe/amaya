...
def test_x_and_not_x(automaton_cls: NFA):
    x_nfa = automaton_cls.for_bool_variable(overall_alphabet=alphabet, var=Var(1), var_value=True)
    not_x_nfa = automaton_cls.for_bool_variable(overall_alphabet=alphabet, var=Var(1), var_value=True).determinize().complement()
...
def test_not_x(automaton_cls: NFA):
    nfa: NFA = automaton_cls.for_bool_variable(alphabet, var=Var(1), var_value=False)
