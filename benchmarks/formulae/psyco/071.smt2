(set-info :smt-lib-version 2.6)
(set-logic LIA)
(set-info
  :source |
 Generated by PSyCO 0.1
 More info in N. P. Lopes and J. Monteiro. Weakest Precondition Synthesis for
 Compiler Optimizations, VMCAI'14.
|)
(set-info :category "industrial")
(set-info :status sat)
(declare-fun R_E1_V4 () Bool)
(declare-fun W_S1_V4 () Bool)
(declare-fun R_E1_V5 () Bool)
(declare-fun W_S1_V5 () Bool)
(declare-fun W_S2_V5 () Bool)
(declare-fun W_S2_V2 () Bool)
(declare-fun W_S2_V3 () Bool)
(declare-fun W_S2_V1 () Bool)
(declare-fun R_S2_V4 () Bool)
(declare-fun R_S2_V5 () Bool)
(declare-fun R_S2_V2 () Bool)
(declare-fun R_S2_V3 () Bool)
(declare-fun R_S2_V1 () Bool)
(declare-fun R_S1_V4 () Bool)
(declare-fun R_S1_V5 () Bool)
(declare-fun R_S1_V2 () Bool)
(declare-fun R_S1_V3 () Bool)
(declare-fun R_S1_V1 () Bool)
(declare-fun R_E1_V2 () Bool)
(declare-fun DISJ_W_S2_R_E1 () Bool)
(declare-fun DISJ_W_S2_R_S2 () Bool)
(declare-fun DISJ_W_S2_R_S1 () Bool)
(declare-fun DISJ_W_S1_W_S2 () Bool)
(declare-fun R_E1_V3 () Bool)
(declare-fun DISJ_W_S1_R_S2 () Bool)
(declare-fun DISJ_W_S1_R_S1 () Bool)
(declare-fun W_S2_V4 () Bool)
(declare-fun W_S1_V3 () Bool)
(declare-fun W_S1_V2 () Bool)
(declare-fun W_S1_V1 () Bool)
(declare-fun R_E1_V1 () Bool)
(declare-fun DISJ_W_S1_R_E1 () Bool)
(assert
 (let
 (($x3432
   (forall
    ((V3_0 Int) (V2_0 Int) 
     (V5_0 Int) (V4_0 Int) 
     (MW_S1_V1 Bool) (MW_S1_V3 Bool) 
     (MW_S1_V2 Bool) (MW_S1_V5 Bool) 
     (MW_S1_V4 Bool) (MW_S2_V1 Bool) 
     (MW_S2_V3 Bool) (MW_S2_V2 Bool) 
     (MW_S2_V5 Bool) (MW_S2_V4 Bool) 
     (S1_V1_!2447 Int) (S1_V1_!2464 Int) 
     (S1_V1_!2474 Int) (S2_V5_!2456 Int) 
     (S2_V5_!2461 Int) (S2_V5_!2472 Int) 
     (S2_V5_!2482 Int) (S1_V3_!2448 Int) 
     (S1_V3_!2465 Int) (S1_V3_!2475 Int) 
     (S1_V2_!2449 Int) (S1_V2_!2466 Int) 
     (S1_V2_!2476 Int) (E1_!2446 Int) 
     (E1_!2452 Int) (E1_!2463 Int) 
     (S2_V4_!2457 Int) (S2_V4_!2462 Int) 
     (S2_V4_!2473 Int) (S2_V4_!2483 Int) 
     (S1_V5_!2450 Int) (S1_V5_!2467 Int) 
     (S1_V5_!2477 Int) (S2_V1_!2453 Int) 
     (S2_V1_!2458 Int) (S2_V1_!2469 Int) 
     (S2_V1_!2479 Int) (S1_V4_!2451 Int) 
     (S1_V4_!2468 Int) (S1_V4_!2478 Int) 
     (S2_V2_!2455 Int) (S2_V2_!2460 Int) 
     (S2_V2_!2471 Int) (S2_V2_!2481 Int) 
     (S2_V3_!2454 Int) (S2_V3_!2459 Int) 
     (S2_V3_!2470 Int) (S2_V3_!2480 Int))
    (let
    (($x3002
      (= (ite MW_S2_V4 S2_V4_!2462 (ite MW_S1_V4 S1_V4_!2451 V4_0))
      (ite MW_S2_V4 S2_V4_!2483
      (ite MW_S1_V4 S1_V4_!2478
      (ite MW_S2_V4 S2_V4_!2473 (ite MW_S1_V4 S1_V4_!2468 V4_0)))))))
    (let
    (($x2713
      (= (ite MW_S2_V5 S2_V5_!2461 (ite MW_S1_V5 S1_V5_!2450 V5_0))
      (ite MW_S2_V5 S2_V5_!2482
      (ite MW_S1_V5 S1_V5_!2477
      (ite MW_S2_V5 S2_V5_!2472 (ite MW_S1_V5 S1_V5_!2467 V5_0)))))))
    (let
    (($x2527
      (= (ite MW_S2_V2 S2_V2_!2460 (ite MW_S1_V2 S1_V2_!2449 V2_0))
      (ite MW_S2_V2 S2_V2_!2481
      (ite MW_S1_V2 S1_V2_!2476
      (ite MW_S2_V2 S2_V2_!2471 (ite MW_S1_V2 S1_V2_!2466 V2_0)))))))
    (let
    (($x2880
      (= (ite MW_S2_V3 S2_V3_!2459 (ite MW_S1_V3 S1_V3_!2448 V3_0))
      (ite MW_S2_V3 S2_V3_!2480
      (ite MW_S1_V3 S1_V3_!2475
      (ite MW_S2_V3 S2_V3_!2470 (ite MW_S1_V3 S1_V3_!2465 V3_0)))))))
    (let ((?x3143 (ite MW_S1_V1 S1_V1_!2464 E1_!2463)))
    (let ((?x2728 (ite MW_S2_V1 S2_V1_!2469 ?x3143)))
    (let ((?x3112 (+ 1 ?x2728)))
    (let ((?x2839 (ite MW_S1_V1 S1_V1_!2474 ?x3112)))
    (let
    (($x3407
      (= (ite MW_S2_V1 S2_V1_!2458 (+ 1 (ite MW_S2_V1 S2_V1_!2453 E1_!2452)))
      (ite MW_S2_V1 S2_V1_!2479 ?x2839))))
    (let
    ((?x2771
      (+ (- 1)
      (ite MW_S2_V2 S2_V2_!2481
      (ite MW_S1_V2 S1_V2_!2476
      (ite MW_S2_V2 S2_V2_!2471 (ite MW_S1_V2 S1_V2_!2466 V2_0)))))))
    (let
    (($x2684
      (>=
      (ite MW_S2_V1 S2_V1_!2458 (+ 1 (ite MW_S2_V1 S2_V1_!2453 E1_!2452)))
      (+ (- 1) (ite MW_S2_V2 S2_V2_!2460 (ite MW_S1_V2 S1_V2_!2449 V2_0))))))
    (let ((?x3089 (ite MW_S2_V1 S2_V1_!2453 E1_!2452)))
    (let ((?x2757 (+ 1 ?x3089)))
    (let ((?x2857 (ite MW_S1_V2 S1_V2_!2449 V2_0)))
    (let ((?x3134 (ite MW_S2_V2 S2_V2_!2455 ?x2857)))
    (let
    (($x2689
      (and (not (<= V2_0 E1_!2446))
      (>= (ite MW_S1_V1 S1_V1_!2447 E1_!2446) (+ (- 1) ?x2857))
      (not (<= ?x2857 E1_!2452)) 
      (not (<= ?x3134 ?x2757)) $x2684 
      (not (<= V2_0 E1_!2463))
      (not
      (<= (ite MW_S2_V2 S2_V2_!2471 (ite MW_S1_V2 S1_V2_!2466 V2_0)) ?x3112))
      (>= (ite MW_S2_V1 S2_V1_!2479 ?x2839) ?x2771))))
    (let ((?x2872 (ite MW_S1_V4 S1_V4_!2468 V4_0)))
    (let ((?x2588 (ite MW_S2_V4 S2_V4_!2473 ?x2872)))
    (let ((?x2488 (ite MW_S1_V4 S1_V4_!2478 ?x2588)))
    (let (($x407 (not R_S2_V4)))
    (let ((?x2789 (ite MW_S1_V5 S1_V5_!2467 V5_0)))
    (let ((?x2967 (ite MW_S2_V5 S2_V5_!2472 ?x2789)))
    (let ((?x3086 (ite MW_S1_V5 S1_V5_!2477 ?x2967)))
    (let (($x405 (not R_S2_V5)))
    (let ((?x2948 (ite MW_S1_V2 S1_V2_!2466 V2_0)))
    (let ((?x3388 (ite MW_S2_V2 S2_V2_!2471 ?x2948)))
    (let ((?x3300 (ite MW_S1_V2 S1_V2_!2476 ?x3388)))
    (let (($x403 (not R_S2_V2)))
    (let ((?x2700 (ite MW_S1_V3 S1_V3_!2465 V3_0)))
    (let ((?x2989 (ite MW_S2_V3 S2_V3_!2470 ?x2700)))
    (let ((?x2722 (ite MW_S1_V3 S1_V3_!2475 ?x2989)))
    (let (($x401 (not R_S2_V3)))
    (let
    (($x3852
      (and (or (not R_S2_V1) (= ?x2839 ?x3143)) 
      (or $x401 (= ?x2722 ?x2700)) 
      (or $x403 (= ?x3300 ?x2948)) 
      (or $x405 (= ?x3086 ?x2789)) 
      (or $x407 (= ?x2488 ?x2872)))))
    (let (($x3851 (not $x3852)))
    (let
    (($x4029
      (and (or (not R_S2_V1) (= ?x2839 ?x2757))
      (or $x401
      (= ?x2722 (ite MW_S2_V3 S2_V3_!2454 (ite MW_S1_V3 S1_V3_!2448 V3_0))))
      (or $x403 (= ?x3300 ?x3134))
      (or $x405
      (= ?x3086 (ite MW_S2_V5 S2_V5_!2456 (ite MW_S1_V5 S1_V5_!2450 V5_0))))
      (or $x407
      (= ?x2488 (ite MW_S2_V4 S2_V4_!2457 (ite MW_S1_V4 S1_V4_!2451 V4_0)))))))
    (let (($x3721 (not $x4029)))
    (let
    (($x2439
      (and (or (not R_S2_V1) (= ?x3143 ?x2757))
      (or $x401
      (= ?x2700 (ite MW_S2_V3 S2_V3_!2454 (ite MW_S1_V3 S1_V3_!2448 V3_0))))
      (or $x403 (= ?x2948 ?x3134))
      (or $x405
      (= ?x2789 (ite MW_S2_V5 S2_V5_!2456 (ite MW_S1_V5 S1_V5_!2450 V5_0))))
      (or $x407
      (= ?x2872 (ite MW_S2_V4 S2_V4_!2457 (ite MW_S1_V4 S1_V4_!2451 V4_0)))))))
    (let (($x3268 (not $x2439)))
    (let
    (($x2744
      (and (or (not R_S2_V1) (= E1_!2452 ?x2839))
      (or $x401 (= (ite MW_S1_V3 S1_V3_!2448 V3_0) ?x2722))
      (or $x403 (= ?x2857 ?x3300))
      (or $x405 (= (ite MW_S1_V5 S1_V5_!2450 V5_0) ?x3086))
      (or $x407 (= (ite MW_S1_V4 S1_V4_!2451 V4_0) ?x2488)))))
    (let (($x3191 (not $x2744)))
    (let
    (($x3674
      (and (or (not R_S2_V1) (= E1_!2452 ?x3143))
      (or $x401 (= (ite MW_S1_V3 S1_V3_!2448 V3_0) ?x2700))
      (or $x403 (= ?x2857 ?x2948))
      (or $x405 (= (ite MW_S1_V5 S1_V5_!2450 V5_0) ?x2789))
      (or $x407 (= (ite MW_S1_V4 S1_V4_!2451 V4_0) ?x2872)))))
    (let (($x3678 (not $x3674)))
    (let
    (($x3705
      (or $x407
      (= (ite MW_S1_V4 S1_V4_!2451 V4_0)
      (ite MW_S2_V4 S2_V4_!2457 (ite MW_S1_V4 S1_V4_!2451 V4_0))))))
    (let
    (($x3221
      (or $x405
      (= (ite MW_S1_V5 S1_V5_!2450 V5_0)
      (ite MW_S2_V5 S2_V5_!2456 (ite MW_S1_V5 S1_V5_!2450 V5_0))))))
    (let
    (($x3557
      (or $x401
      (= (ite MW_S1_V3 S1_V3_!2448 V3_0)
      (ite MW_S2_V3 S2_V3_!2454 (ite MW_S1_V3 S1_V3_!2448 V3_0))))))
    (let
    (($x2729
      (and (or (not R_S2_V1) (= E1_!2452 ?x2757)) $x3557
      (or $x403 (= ?x2857 ?x3134)) $x3221 $x3705)))
    (let (($x2507 (not $x2729)))
    (let
    (($x3264
      (and (or (not R_S2_V1) (= ?x2839 E1_!2452))
      (or $x401 (= ?x2722 (ite MW_S1_V3 S1_V3_!2448 V3_0)))
      (or $x403 (= ?x3300 ?x2857))
      (or $x405 (= ?x3086 (ite MW_S1_V5 S1_V5_!2450 V5_0)))
      (or $x407 (= ?x2488 (ite MW_S1_V4 S1_V4_!2451 V4_0))))))
    (let
    (($x2571
      (and (or (not R_S2_V1) (= ?x3143 ?x2839)) 
      (or $x401 (= ?x2700 ?x2722)) 
      (or $x403 (= ?x2948 ?x3300)) 
      (or $x405 (= ?x2789 ?x3086)) 
      (or $x407 (= ?x2872 ?x2488)))))
    (let
    (($x3363
      (and (or (not R_S2_V1) (= ?x3143 E1_!2452))
      (or $x401 (= ?x2700 (ite MW_S1_V3 S1_V3_!2448 V3_0)))
      (or $x403 (= ?x2948 ?x2857))
      (or $x405 (= ?x2789 (ite MW_S1_V5 S1_V5_!2450 V5_0)))
      (or $x407 (= ?x2872 (ite MW_S1_V4 S1_V4_!2451 V4_0))))))
    (let
    (($x3054
      (and (or (not R_S1_V1) (= ?x2728 (+ (- 1) E1_!2446)))
      (or (not R_S1_V3) (= ?x2989 V3_0)) 
      (or (not R_S1_V2) (= ?x3388 V2_0)) 
      (or (not R_S1_V5) (= ?x2967 V5_0)) 
      (or (not R_S1_V4) (= ?x2588 V4_0)))))
    (let (($x2831 (not $x3054)))
    (let
    (($x3193
      (and (or (not R_S1_V1) (= E1_!2463 ?x3112))
      (or (not R_S1_V3) (= V3_0 ?x2989)) 
      (or (not R_S1_V2) (= V2_0 ?x3388)) 
      (or (not R_S1_V5) (= V5_0 ?x2967)) 
      (or (not R_S1_V4) (= V4_0 ?x2588)))))
    (let (($x2923 (not $x3193)))
    (let
    (($x2558
      (and (or (not R_S2_V1) (= ?x3089 (+ (- 1) ?x2839)))
      (or $x401
      (= (ite MW_S2_V3 S2_V3_!2454 (ite MW_S1_V3 S1_V3_!2448 V3_0)) ?x2722))
      (or $x403 (= ?x3134 ?x3300))
      (or $x405
      (= (ite MW_S2_V5 S2_V5_!2456 (ite MW_S1_V5 S1_V5_!2450 V5_0)) ?x3086))
      (or $x407
      (= (ite MW_S2_V4 S2_V4_!2457 (ite MW_S1_V4 S1_V4_!2451 V4_0)) ?x2488)))))
    (let
    (($x2800
      (and (or (not R_S1_V1) (= ?x2728 (+ (- 1) E1_!2463)))
      (or (not R_S1_V3) (= ?x2989 V3_0)) 
      (or (not R_S1_V2) (= ?x3388 V2_0)) 
      (or (not R_S1_V5) (= ?x2967 V5_0)) 
      (or (not R_S1_V4) (= ?x2588 V4_0)))))
    (let
    (($x2460
      (and (or (not R_S2_V1) (= ?x3089 (+ (- 1) ?x3143)))
      (or $x401
      (= (ite MW_S2_V3 S2_V3_!2454 (ite MW_S1_V3 S1_V3_!2448 V3_0)) ?x2700))
      (or $x403 (= ?x3134 ?x2948))
      (or $x405
      (= (ite MW_S2_V5 S2_V5_!2456 (ite MW_S1_V5 S1_V5_!2450 V5_0)) ?x2789))
      (or $x407
      (= (ite MW_S2_V4 S2_V4_!2457 (ite MW_S1_V4 S1_V4_!2451 V4_0)) ?x2872)))))
    (let
    (($x2934
      (and (or (not R_E1_V2) (= ?x2857 V2_0))
      (or (not R_E1_V5) (= (ite MW_S1_V5 S1_V5_!2450 V5_0) V5_0))
      (or (not R_E1_V4) (= (ite MW_S1_V4 S1_V4_!2451 V4_0) V4_0)))))
    (let
    (($x3393
      (or $x407
      (= (ite MW_S2_V4 S2_V4_!2457 (ite MW_S1_V4 S1_V4_!2451 V4_0))
      (ite MW_S1_V4 S1_V4_!2451 V4_0)))))
    (let
    (($x2451
      (or $x405
      (= (ite MW_S2_V5 S2_V5_!2456 (ite MW_S1_V5 S1_V5_!2450 V5_0))
      (ite MW_S1_V5 S1_V5_!2450 V5_0)))))
    (let
    (($x2899
      (or $x401
      (= (ite MW_S2_V3 S2_V3_!2454 (ite MW_S1_V3 S1_V3_!2448 V3_0))
      (ite MW_S1_V3 S1_V3_!2448 V3_0)))))
    (let
    (($x3332
      (and (or (not R_S2_V1) (= ?x3089 (+ (- 1) E1_!2452))) $x2899
      (or $x403 (= ?x3134 ?x2857)) $x2451 $x3393)))
    (let
    (($x3695
      (and (or (not R_S1_V1) (= E1_!2446 ?x3112))
      (or (not R_S1_V3) (= V3_0 ?x2989)) 
      (or (not R_S1_V2) (= V2_0 ?x3388)) 
      (or (not R_S1_V5) (= V5_0 ?x2967)) 
      (or (not R_S1_V4) (= V4_0 ?x2588)))))
    (let
    (($x3050
      (and (or (not $x3695) (= S1_V1_!2447 S1_V1_!2474))
      (or (not (or (not R_S1_V1) (= E1_!2463 E1_!2446)))
      (= S1_V1_!2464 S1_V1_!2447)) 
      (or $x2923 (= S1_V1_!2464 S1_V1_!2474))
      (or $x3678 (= S2_V5_!2456 S2_V5_!2472))
      (or (not $x3332) (= S2_V5_!2461 S2_V5_!2456))
      (or (not $x2460) (= S2_V5_!2461 S2_V5_!2472))
      (or (not $x3264) (= S2_V5_!2482 S2_V5_!2456))
      (or $x3721 (= S2_V5_!2482 S2_V5_!2461))
      (or $x3851 (= S2_V5_!2482 S2_V5_!2472))
      (or (not (or (not R_S1_V1) (= E1_!2463 E1_!2446)))
      (= S1_V3_!2465 S1_V3_!2448)) 
      (or $x2923 (= S1_V3_!2465 S1_V3_!2475))
      (or $x2831 (= S1_V3_!2475 S1_V3_!2448))
      (or (not (or (not R_S1_V1) (= E1_!2463 E1_!2446)))
      (= S1_V2_!2466 S1_V2_!2449)) 
      (or $x2831 (= S1_V2_!2476 S1_V2_!2449))
      (or (not $x2800) (= S1_V2_!2476 S1_V2_!2466))
      (or (not $x2934) (= E1_!2452 E1_!2446))
      (or (not $x2934) (= E1_!2452 E1_!2463)) 
      (= E1_!2463 E1_!2446) 
      (or $x2507 (= S2_V4_!2457 S2_V4_!2462))
      (or $x3678 (= S2_V4_!2457 S2_V4_!2473))
      (or $x3191 (= S2_V4_!2457 S2_V4_!2483))
      (or (not $x2460) (= S2_V4_!2462 S2_V4_!2473))
      (or (not $x2558) (= S2_V4_!2462 S2_V4_!2483))
      (or $x3851 (= S2_V4_!2483 S2_V4_!2473))
      (or (not (or (not R_S1_V1) (= E1_!2463 E1_!2446)))
      (= S1_V5_!2467 S1_V5_!2450)) 
      (or $x2831 (= S1_V5_!2477 S1_V5_!2450))
      (or (not $x2800) (= S1_V5_!2477 S1_V5_!2467))
      (or $x2507 (= S2_V1_!2453 S2_V1_!2458))
      (or $x3191 (= S2_V1_!2453 S2_V1_!2479))
      (or (not $x2558) (= S2_V1_!2458 S2_V1_!2479))
      (or (not $x3363) (= S2_V1_!2469 S2_V1_!2453))
      (or $x3268 (= S2_V1_!2469 S2_V1_!2458))
      (or (not $x2571) (= S2_V1_!2469 S2_V1_!2479))
      (or (not (or (not R_S1_V1) (= E1_!2463 E1_!2446)))
      (= S1_V4_!2468 S1_V4_!2451)) 
      (or $x2923 (= S1_V4_!2468 S1_V4_!2478))
      (or $x2831 (= S1_V4_!2478 S1_V4_!2451))
      (or $x2507 (= S2_V2_!2455 S2_V2_!2460))
      (or (not $x3363) (= S2_V2_!2471 S2_V2_!2455))
      (or $x3268 (= S2_V2_!2471 S2_V2_!2460))
      (or (not $x2571) (= S2_V2_!2471 S2_V2_!2481))
      (or (not $x3264) (= S2_V2_!2481 S2_V2_!2455))
      (or $x3721 (= S2_V2_!2481 S2_V2_!2460))
      (or $x2507 (= S2_V3_!2454 S2_V3_!2459))
      (or $x3678 (= S2_V3_!2454 S2_V3_!2470))
      (or $x3191 (= S2_V3_!2454 S2_V3_!2480))
      (or $x3268 (= S2_V3_!2470 S2_V3_!2459))
      (or $x3721 (= S2_V3_!2480 S2_V3_!2459))
      (or $x3851 (= S2_V3_!2480 S2_V3_!2470)) 
      (not MW_S1_V1) (not MW_S1_V2) 
      (or (not MW_S1_V5) W_S1_V5) 
      (or (not MW_S1_V4) W_S1_V4) 
      (or (not MW_S2_V1) W_S2_V1) 
      (or (not MW_S2_V3) W_S2_V3) 
      (or (not MW_S2_V2) W_S2_V2) 
      (or (not MW_S2_V5) W_S2_V5))))
    (or (not $x3050) (not $x2689) (and $x3407 $x2880 $x2527 $x2713 $x3002)))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))
 (let
 (($x3362 (not (or (and W_S2_V2 R_E1_V2) (and W_S2_V5 R_E1_V5) R_E1_V4))))
 (let
 (($x113
   (or (and W_S2_V1 R_S2_V1) 
   (and W_S2_V3 R_S2_V3) (and W_S2_V2 R_S2_V2) 
   (and W_S2_V5 R_S2_V5) R_S2_V4)))
 (let (($x115 (= DISJ_W_S2_R_S2 (not $x113))))
 (let
 (($x110
   (or (and W_S2_V1 R_S1_V1) 
   (and W_S2_V3 R_S1_V3) (and W_S2_V2 R_S1_V2) 
   (and W_S2_V5 R_S1_V5) R_S1_V4)))
 (let (($x112 (= DISJ_W_S2_R_S1 (not $x110))))
 (let
 (($x3360
   (= DISJ_W_S1_W_S2 (not (or W_S2_V3 (and W_S1_V5 W_S2_V5) W_S1_V4)))))
 (let (($x155 (not R_E1_V3)))
 (let
 (($x3008 (not (or R_S2_V3 (and W_S1_V5 R_S2_V5) (and W_S1_V4 R_S2_V4)))))
 (let
 (($x3307 (not (or R_S1_V3 (and W_S1_V5 R_S1_V5) (and W_S1_V4 R_S1_V4)))))
 (let
 (($x3290
   (or (and (not W_S2_V2) DISJ_W_S2_R_S1 DISJ_W_S1_R_S1)
   (and (not W_S2_V1) (not W_S2_V2)) 
   (and (not R_S1_V1) DISJ_W_S1_R_S1))))
 (let (($x2350 (not W_S1_V2)))
 (let (($x2301 (not W_S1_V1)))
 (let (($x153 (not R_E1_V1)))
 (and DISJ_W_S1_R_E1 $x153 $x2301 $x2350 $x3290 W_S1_V3 W_S2_V4
 (= DISJ_W_S1_R_S1 $x3307) 
 (= DISJ_W_S1_R_S2 $x3008) $x155 $x3360 $x112 $x115 
 (= DISJ_W_S2_R_E1 $x3362) $x3432 
 (not (and W_S1_V5 R_E1_V5)) 
 (not (and W_S1_V4 R_E1_V4))))))))))))))))))
(check-sat)
(exit)

